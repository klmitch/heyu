...
class NotificationServer(object):
    """
    Notification server.  Notifiers instantiate this class, then
    iterate over it to retrieve the actual notifications, which they
    then handle.
    """
    ...
    def _acceptor(self, tend):
        ...
        :returns: An instance of ``NotificationApplication``.
        """

        # Set up the application
        self._hub_app = NotificationApplication(tend, self, self._app_name,
                                                self._app_id)
...
class NotificationApplication(tendril.Application):
    """
    ...
    def __init__(self, parent, server, app_name, app_id):
        """
        Initialize a HeyU notification application.

        :param parent: The parent of the ``NotificationApplication``.
                       This will be an instance of
                       ``tendril.Tendril``.
        :param server: The underlying HeyU notification server
                       instance.  The server provides an iterator
                       interface for the use of notifiers.
        ...
        super(NotificationApplication, self).__init__(parent)
...
@cli_tools.console
def stdout_notifier(hub, cert_conf=None, secure=True):
    """
    ...
    # Set up the server
    server = NotificationServer(hub, cert_conf, secure)
...
def file_notifier(filename, hub, cert_conf=None, secure=True):
    """
...
@cli_tools.argument('script',
                    ...
                    'interpretation.')
def script_notifier(script, hub, cert_conf=None, secure=True):
    """
